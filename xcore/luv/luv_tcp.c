...
uv_tcp_t* luv_tcp_check(lua_State* L, int index) {
	uv_tcp_t* handle = (uv_tcp_t*)luaL_checkudata(L, index, "uv_tcp");
	if (handle) {
...
int luv_tcp_open(lua_State* L) {
	uv_tcp_t* handle = luv_tcp_check(L, 1);
...
int luv_tcp_connect(lua_State* L) {
	...
	req = (uv_connect_t*)lua_newuserdata(L, sizeof(uv_connect_t));
	req->data = luv_req_setup(L, ref);
	ret = uv_tcp_connect(req, handle, (struct sockaddr*)&addr, luv_tcp_connect_cb);
	if (ret < 0) {
		lua_pop(L, 1);
		return luv_error(L, ret);
	}
