...
size_t lmz_file_read(void *pOpaque, mz_uint64 file_ofs, void *pBuf, size_t n) {
  lmz_file_t* zip = (lmz_file_t*)pOpaque;
  const uv_buf_t buf = uv_buf_init((char*)pBuf, n);
  uv_fs_read(zip->loop, &(zip->req), zip->fd, &buf, 1, file_ofs, NULL);
  return zip->req.result;
}

int lmz_reader_init(lua_State* L) {
  const char* path = luaL_checkstring(L, 1);
  mz_uint32 flags = luaL_optinteger(L, 2, 0);
  mz_uint64 size;
  lmz_file_t* zip = (lmz_file_t*)lua_newuserdata(L, sizeof(lmz_file_t));
...
int lmz_reader_gc(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
...
int lmz_writer_gc(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_writer");
  if (zip == NULL) { return 0; }
  mz_zip_writer_end(&(zip->archive));
  return 0;
}

int lmz_reader_get_num_files(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
...
int lmz_reader_locate_file(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
...
int lmz_reader_stat(lua_State* L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
...
int lmz_reader_get_filename(lua_State* L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
  mz_uint file_index = luaL_checkinteger(L, 2) - 1;
...
int lmz_reader_is_file_a_directory(lua_State  *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
  mz_uint file_index = luaL_checkinteger(L, 2) - 1;
...
int lmz_reader_extract(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
  mz_uint file_index = luaL_checkinteger(L, 2) - 1;
  mz_uint flags = luaL_optinteger(L, 3, 0);
  size_t out_len;
  char* out_buf = (char*)mz_zip_reader_extract_to_heap(&(zip->archive), file_index, &out_len, flags);
  lua_pushlstring(L, out_buf, out_len);
  free(out_buf);
  return 1;
}

int lmz_reader_get_offset(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_reader");
  if (zip == NULL) { return 0; }
  mz_zip_archive* archive = &(zip->archive);
...
int lmz_writer_init(lua_State *L) {
...
int lmz_writer_add_from_zip_reader(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_writer");
  if (zip == NULL) { return 0; }
  lmz_file_t* source = (lmz_file_t*)luaL_checkudata(L, 2, "miniz_reader");
...
int lmz_writer_add_mem(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_writer");
  if (zip == NULL) { return 0; }
  const char* path = luaL_checkstring(L, 2);
  size_t size;
...
int lmz_writer_finalize(lua_State *L) {
  lmz_file_t* zip = (lmz_file_t*)luaL_checkudata(L, 1, "miniz_writer");
  if (zip == NULL) { return 0; }
  void* data;
  size_t size;
...
int ltinfl(lua_State* L) {
  size_t in_len;
  const char* in_buf = luaL_checklstring(L, 1, &in_len);
  size_t out_len;
  int flags = luaL_optinteger(L, 2, 0);
  char* out_buf = (char*)tinfl_decompress_mem_to_heap(in_buf, in_len, &out_len, flags);
  lua_pushlstring(L, out_buf, out_len);
  free(out_buf);
  return 1;
}

int ltdefl(lua_State* L) {
  size_t in_len;
  const char* in_buf = luaL_checklstring(L, 1, &in_len);
  size_t out_len;
  int flags = luaL_optinteger(L, 2, 0);
  char* out_buf = (char*)tdefl_compress_mem_to_heap(in_buf, in_len, &out_len, flags);
  lua_pushlstring(L, out_buf, out_len);
  free(out_buf);
  return 1;
}

const luaL_Reg lminiz_read_m[] = {
...
void miniz_loadfile(lua_State *L, const char* path, const char* file) {
	// 构建压缩包
	mz_uint64 size;
	lmz_file_t* zip = (lmz_file_t*)lua_newuserdata(L, sizeof(lmz_file_t));
	mz_zip_archive* archive = &(zip->archive);
	...
	// 查询文件索引
	int index = mz_zip_reader_locate_file(archive, file, NULL, 0);
	if (index < 0) {
		return;
	}
	...
	// 获取文件内容
	size_t out_len;
	char* out_buf = (char*)mz_zip_reader_extract_to_heap(&(zip->archive), index, &out_len, 0);
	if (!out_buf) {
		return;
	}
	out_buf[out_len] = '\0';
