...
uv_fs_t* luv_fs_check(lua_State* L, int index) {
...
void luv_fs_cb(uv_fs_t* req) {
  ...
  luv_req_fulfill(L, (luv_req_t*)(req->data), nargs);
...
int luv_fs_close(lua_State* L) {
  ...
  uv_fs_t* req = (uv_fs_t*)lua_newuserdata(L, sizeof(uv_fs_t));
...
int luv_fs_open(lua_State* L) {
  ...
  int flags = luv_fs_check_flags(L, 2);
  int mode = luaL_checkinteger(L, 3);
  int ref = luv_req_check_continuation(L, 4);
  uv_fs_t* req = (uv_fs_t*)lua_newuserdata(L, sizeof(uv_fs_t));
  req->data = luv_req_setup(L, ref);
...
int luv_fs_read(lua_State* L) {
  ...
  req = (uv_fs_t*)lua_newuserdata(L, sizeof(uv_fs_t));
  req->data = luv_req_setup(L, ref);
...
int luv_fs_write(lua_State* L) {
  ...
  if (lua_istable(L, 2)) {
	  bufs = luv_stream_prep_bufs(L, 2, &count);
  }
...
int luv_fs_sendfile(lua_State* L) {
  ...
  int ref = luv_req_check_continuation(L, 5);
  uv_fs_t* req = (uv_fs_t*)lua_newuserdata(L, sizeof(uv_fs_t));
  req->data = luv_req_setup(L, ref);
...
int luv_fs_access(lua_State* L) {
  const char* path = luaL_checkstring(L, 1);
  int amode = luv_fs_check_amode(L, 2);
  int ref = luv_req_check_continuation(L, 3);
  uv_fs_t* req = (uv_fs_t*)lua_newuserdata(L, sizeof(uv_fs_t));
  req->data = luv_req_setup(L, ref);
