...
int luv_process_disable_stdio_inheritance(lua_State* L) {
	uv_disable_stdio_inheritance();
	return 0;
}

uv_process_t* luv_process_check(lua_State* L, int index) {
	uv_process_t* handle = (uv_process_t*)luaL_checkudata(L, index, "uv_process");
	if (handle) {
...
void exit_cb(uv_process_t* handle, int64_t exit_status, int term_signal) {
	...
	luv_callback_call(L, (luv_handle_t*)(handle->data), LUV_EXIT, 2);
}

void luv_spawn_close_cb(uv_handle_t* handle) {
	lua_State *L = luv_state(handle->loop);
	luv_handle_cleanup(L, (luv_handle_t*)(handle->data));
}

void luv_clean_options(uv_process_options_t* options) {
	free(options->args);
	free(options->stdio);
	free(options->env);
}

int luv_process_spawn(lua_State* L) {
  uv_process_t* handle;
  ...
  if (lua_type(L, -1) == LUA_TTABLE) {
    ...
    options.stdio = (uv_stdio_container_t*)malloc(len * sizeof(uv_stdio_container_t));
    if (!options.stdio) {
      luv_clean_options(&options);
    ...
    for (i = 0; i < len; ++i) {
      ...
      // userdata is assumed to be a uv_stream_t instance
        ...
        uv_os_fd_t fd;
        uv_stream_t* stream = luv_stream_check(L, -1);
        int err = uv_fileno((uv_handle_t*)stream, &fd);
        if (err == UV__EINVAL || err == UV__EBADF) {
          options.stdio[i].flags = (uv_stdio_flags)(UV_CREATE_PIPE | UV_READABLE_PIPE | UV_WRITABLE_PIPE);
        }
        else {
  ...
  if (!lua_isnoneornil(L, 3)) {
    luv_callback_check(L, (luv_handle_t*)(handle->data), LUV_EXIT, 3);
  }
...
int luv_parse_signal(lua_State* L, int slot) {
...
int luv_process_kill(lua_State* L) {
  uv_process_t* handle = luv_process_check(L, 1);
