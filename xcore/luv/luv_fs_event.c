...
uv_fs_event_t* luv_fs_event_check(lua_State* L, int index) {
	uv_fs_event_t* handle = (uv_fs_event_t*)luaL_checkudata(L, index, "uv_fs_event");
	if (!handle) return NULL;
	luaL_argcheck(L, handle->type == UV_FS_EVENT && handle->data, index, "Expected uv_fs_event_t");
	return handle;
}

int luv_fs_event_new(lua_State* L) {
...
void luv_fs_event_cb(uv_fs_event_t* handle, const char* filename, int events, int status) {
  ...
  luv_callback_call(L, (luv_handle_t*)(handle->data), LUV_FS_EVENT, 3);
}

int luv_fs_event_start(lua_State* L) {
  uv_fs_event_t* handle = luv_fs_event_check(L, 1);
  ...
  lua_pop(L, 1);
  luv_callback_check(L, (luv_handle_t*)(handle->data), LUV_FS_EVENT, 4);
...
int luv_fs_event_stop(lua_State* L) {
  uv_fs_event_t* handle = luv_fs_event_check(L, 1);
...
int luv_fs_event_getpath(lua_State* L) {
  uv_fs_event_t* handle = luv_fs_event_check(L, 1);
